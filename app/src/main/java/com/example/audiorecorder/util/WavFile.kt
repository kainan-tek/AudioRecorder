...
class WavFile(private val filePath: String) {
    ...
    fun create(sampleRate: Int, channelCount: Int, bitsPerSample: Int): Boolean {
        ...
        return try {
            ...
            Log.i(
                TAG,
                "WAV file created successfully: ${sampleRate}Hz, ${getChannelDescription()}ch, ${bitsPerSample}bit"
            )
            true
    ...
    fun close(): Boolean {
        if (isFileOpen || fileOutputStream != null) {
            Log.d(
                TAG, "Closing WAV file: $totalAudioLength bytes, ${
                    String.format(
    ...
    override fun toString(): String {
        return "WavFile(path='$filePath', ${sampleRate}Hz, ${getChannelDescription()}ch, ${bitsPerSample}bit, ${
            String.format(
                java.util.Locale.US, "%.2f", duration
            )
        }s)"
